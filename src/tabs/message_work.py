        ...
        if c_search_no and not db_contract.empty:
            ...
            if not c_match.empty:
                ...
                c_docs = st.multiselect(
                    "📄 포함 서류 선택",
                    ["사용자별 퀵 가이드", "상세 가이드 링크", "서비스 소개서", "학생 계정 명단", "학생 업로드용 양식", "교사 업로드용 양식", "학운위 체크리스트"],
                    default=["사용자별 퀵 가이드", "상세 가이드 링크", "학생 업로드용 양식","교사 업로드용 양식", "학운위 체크리스트"],
                    key="c_docs_sel"
                )
                ...
                if st.button("🚀 정규 계약 안내문 생성", type="primary", use_container_width=True):
                    ...
                    for doc in c_docs:
                        ...
                        if doc not in ["상세 가이드 링크", "학운위 체크리스트", "사용자별 퀵 가이드"]:
                            # 양식 파일 이름 커스텀
                            if doc == "학생 업로드용 양식":
                                a_list.append(f"- [{c_sch}_학생_업로드용] 엑셀 파일")
                            elif doc == "교사 업로드용 양식":
                                a_list.append(f"- [{c_sch}_선생님_업로드용] 엑셀 파일")
                            else:
                                a_list.append(f"- {doc}")
